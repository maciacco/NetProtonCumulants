...
void ReadTreeMix(const char* fname = "newTree_noTOF", const char* ofname = "LHC18pp_noTOF", const int iVarMin = 526, const int iVarMax = 527, const bool isMC = false, const char source = kSources::kPrim)
{
  ...
  TH3F **hGenRecProton[2];
  TH3D **hGenProton[2];
  TH3D **hRecProton[2];
  TH3D **hGenProtonTrkl[2];
  TH3D **hRecProtonTrkl[2];

  for (int i{0}; i < 2; ++i){
    hGenRecProton[i] = new TH3F*[nF];
    hGenProton[i] = new TH3D*[nF];
    hRecProton[i] = new TH3D*[nF];
    hGenProtonTrkl[i] = new TH3D*[nF];
    hRecProtonTrkl[i] = new TH3D*[nF];
  }
  ...
  TH1D hTrklTmp("hTrklTmp", "hTrklTmp", kNTrklBins, kTrklBins);
