...
void Analysis(const char* period = "18", const char* obs = "k2k1")
{
  ...
  for(int iVar = 364; iVar < 365; ++iVar)
  {
    ...
    for(int i = 1; i <= (kMultV0M ? kNCentBins : kNTrklBins); i++)
    {
      ...
      else if (obs_str == "k2sk") cumulant(mean, rms, k2[i - 1], k2sk[i - 1], nSkip);
      else if (obs_str == "k1") cumulant(mean, rms, k2[i - 1], k1[i - 1], nSkip);
      else if (obs_str == "k2") cumulant(mean, rms, k2[i - 1], k2[i - 1], nSkip);
      else if (obs_str == "k3") cumulant(mean, rms, k2[i - 1], k3[i - 1], nSkip);
      else if (obs_str == "k4") cumulant(mean, rms, k2[i - 1], k4[i - 1], nSkip);
      else if (obs_str == "k5") cumulant(mean, rms, k2[i - 1], k5[i - 1], nSkip);
      else if (obs_str == "k6") cumulant(mean, rms, k2[i - 1], k6[i - 1], nSkip);
